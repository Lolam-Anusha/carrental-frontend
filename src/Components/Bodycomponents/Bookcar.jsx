...
export default function Bookcar({ isDivVisible, setDivVisible, carType, pickPlace, dropPlace, pickDate, dropDate }) {
    ...
    const isReserveDesable = !pickTime || !dropTime || !firstname ||
        !lastname || !age || !phone || !email || !address || !city || !zipcode;
    ...
    return (
        <>
            {isDivVisible && (
                <div className="container border-4 border-white shadow-2xl overflow-x-hidden mx-auto bg-white absolute z-50 w-3/5 flex flex-col justify-center items-center"
                    style={{ top: '170%', left: '50%', transform: 'translate(-50%, -50%)' }}>
                    ...
                    <div className="bg-[#ffeae6] justify-start py-4 px-4 w-full">
                        <h2 className="font-bold font-sans text-2xl py-2 text-orange">
                    ...
                    <div className="grid lg:grid-cols-2 w-full border-b">
                        <div className="justify-start py-4 px-4 w-full bg-white">
                            ...
                            <h3 className="py-2 font-bold font-sans text-lg">
                                <CalendarMonthOutlinedIcon className="text-black" /> Pick up Date & Time
                            </h3>
                            <p className="text-base px-2 text-[#777] font-sans">{pickDate} / </p>
                            <input
                                value={pickTime}
                                onChange={ev => setPickTime(ev.target.value)}
                                max={dropTime}
                                className="border-4 focus:outline-none"
                                type="time"
                            />
                            <h3 className="py-2 font-bold font-sans text-lg">
                                <CalendarMonthOutlinedIcon className="text-black" /> Drop off Date & Time
                            </h3>
                            <p className="text-base px-2 text-[#777] font-sans">{dropDate} /</p>
                            <input
                                value={dropTime}
                                onChange={ev => setDropTime(ev.target.value)}
                                min={pickTime}
                                className="border-4 focus:outline-none"
                                type="time"
                            />
                            <h3 className="py-2 font-bold font-sans text-lg">
                                <LocationOnOutlinedIcon className="text-black" /> Pick up Location
                            </h3>
                            <p className="text-base px-2 text-[#777] font-sans">{pickPlace}</p>
                            <h3 className="py-2 font-bold font-sans text-lg">
                                <LocationOnOutlinedIcon className="text-black" /> Drop off Location
                            </h3>
